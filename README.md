...
fojs 是 **Vue 3 的超集**。它以非侵入式的方式深度集成于 Vue 生态，允许你在同一个项目中混合使用 `.vue` 标准组件与 `.fo` 魔法组件。

- **React 体验**：使用 TSX/JSX 构建 UI，支持同文件多组件组合。
- **Vue 性能**：逻辑层直接复用 Vue 3 Composition API 响应式系统。
- **非侵入式**：通过 Vite 插件精准拦截 `.fo` 文件，不干扰现有 Vue 代码，完美兼容所有 Vue 插件与 UI 库。

## 主要特性

- **Vue 超集架构**：完全支持 Vue 生态（Router, Pinia, Element Plus），`.fo` 组件可直接在 `.vue` 中引用，反之亦然。
- **Scoped CSS/SCSS/Less**：支持在 `.fo` 中直接声明样式变量，编译期自动实现作用域隔离。
...
fojs 的设计初衷是**非侵入式**。你可以在现有的 Vue 项目中开启“魔法模式”。
...
```bash
npm install @fo4/fojs
```

### 1）`.fo` 组件的基本写法

`.fo` 的默认导出是一个函数。你可以把它理解成 Vue 组件的 `setup()`：写逻辑、返回 JSX 作为渲染内容。
...
- fojs：`useFoStore/useFoEffect/useVModel`

### 3）子组件写法（同文件组件 / 组合组件）

你可以在同一个 `.fo` 文件里用函数声明子组件，然后像 React 一样在 JSX 里使用：
...
- 只要某个函数变量被当成 `<组件 />` 使用，fojs 会把它自动包装成真正的 Vue 组件实例（支持生命周期）
...
`defineProps` 和 `defineEmits` 由编译器自动注入（无需手动 import），用于在 `.fo` 中显式声明组件的 props 与事件。
...
在 `vite.config.ts` 中，将 `fojs` 插件置于 `vue` 插件之前：

```ts
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
import fojs from '@fo4/fojs'
...
```vue
<script setup>
import MyFoCard from './components/Card.fo'
</script>
<template>
  <MyFoCard title="来自 fojs 的组件" class="custom-style" />
</template>
```
